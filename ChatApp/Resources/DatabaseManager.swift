...
extension DatabaseManager
{
    ...
    {
        database.child(user.safeEmail).setValue(["name": user.name]) { [weak self] error, _ in
            ...
            strongSelf.database.child("users").observeSingleEvent(of: .value) { snapshot in
                if var usersCollection = snapshot.value as? [[String: String]]
                {
                    ...
                    let newElement = ["name": user.name, "email": user.safeEmail]
    ...
    {
        case failedToFetch
        public var localizedDescription: String { switch self { case .failedToFetch: return "This means ... failed" }}
    }
}
...
*/
    ...
    public func createNewConversation(with otherUserEmail: String, name: String, firstMessage: Message, completion: @escaping (Bool) -> Void)
    {
        ...
        ref.observeSingleEvent(of: .value) { [weak self] snapshot in
           ...
           let conversationId = "conversation_\(firstMessage.messageId.replacingOccurrences(of: ".", with: "-").replacingOccurrences(of: "#", with: "-").replacingOccurrences(of: "$", with: "-").replacingOccurrences(of: "[", with: "-").replacingOccurrences(of: "]", with: "-"))"
...
//        }
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                ...
                if type == "photo"
                {
                    ...
                    guard let imageUrl = URL(string: content),
                          let placeHolder = UIImage(named: "Black")
                    else { return nil}
                ...
                else if type == "location"
                {
                    ...
                    guard let longitude = Double(locationComponents[0]), let latitude = Double(locationComponents[1]) else { return nil }
    ...
    public func sendMessage(to conversation: String, otherUserEmail: String, name: String,  newMessage: Message, completion: @escaping (Bool) -> Void)
    {
        ...
        database.child("\(conversation)/messages").observeSingleEvent(of: .value) { [weak self] snapshot in
            ...
            case .text(let messageText): message = messageText
            case .attributedText(_): break
            case .photo(let mediaItem): if let targetUrlString = mediaItem.url?.absoluteString { message = targetUrlString }
                break
            case .video(let mediaItem): if let targetUrlString = mediaItem.url?.absoluteString { message = targetUrlString }
                break
            case .location(let locationData): let location = locationData.location
...
struct ChatAppUser
{
    ...
    {
        //randomuser-gmail-com_profile_picture.png
